// 
//  To compile: c++ abs_anti_mips_spinoff_1d.cc cokus3.c  -o antimips_1d -lgsl -lgslcblas -lm
...
// OUTPUTS VARIABLES NAME ENTER   
	  ...
	  TopeX=atoi(argv[2]);
	//   TopeY=atoi(argv[3]);
	  R = strtod(argv[3], &pEnd);
	  eta = strtod(argv[4], &pEnd);
	    TempsTotal=atoi(argv[5]);
	  TempsInitial=0.7*TempsTotal;
      Dt = strtod(argv[6], &pEnd);
     Vo = strtod(argv[7], &pEnd);
	 gamma = strtod(argv[8], &pEnd);
	...
	double kResort;
    double rand_nbr;
    double dist0=sqrt(TopeX*TopeX)/100;
...
if (argc>1) {
    ...
    //    strcat(filename,"_Ly_"); strcat(filename,argv[3]);
	  strcat(filename,"_R_"); strcat(filename,argv[3]);
      strcat(filename,"_eta_"); strcat(filename,argv[4]);
      strcat(filename,"_T_"); strcat(filename,argv[5]);
      strcat(filename,"_Dt_"); strcat(filename,argv[6]);
      strcat(filename,"_Vo_"); strcat(filename,argv[7]);
	  strcat(filename,"_gamma_"); strcat(filename,argv[8]);
	  strcat(filename,".dat");
...
newRodsAngle[i] = RodsAngle[i] + eta*gsl_ran_gaussian(rg,SqrtDeltaT) + gamma*Alignment*Dt/(1+Neighbors);
...
P=P/N;
    ...
    for(k=0;k<N;k++)
        ...
        //    RodsSpaceY[k]=newRodsSpaceY[k];
		   ...
		   if (t%T_SNAP==0) toXY << RodsSpaceX[k] <<  " " << RodsAngle[k] << "\n";
		   if (t%T_OP==0) toPolar << (t*Dt) << " " << P << " " << Px << " " << Py << "\n";
 		 }
